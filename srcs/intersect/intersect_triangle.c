...
/*   Updated: 2020/11/08 18:08:29 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double
    plane_dist(double closest, t_ray *ray, t_vec3lf *plane_normal)
{
    double      dist;
    ...
    *plane_normal = vecnorm(*plane_normal);
    ...
    dist = -vecdotprod(ray->pos, *plane_normal)
        / vecdotprod(ray->dir, *plane_normal);
    if (dist < 0.1 || dist > closest)
        return (0);
    return (dist);
}
    ...
    intersect_triangle(double *closest, t_ray *ray, void *elem_detail)
{
    ...
    ray->bounce.surface_normal = tr->normal;
    ray->color = tr->color;
    *closest = dist;
    return (1);
}
